...
class CursorSession:
    ...
    is_new_session: bool = False  # True if session hasn't been sent to Cursor yet
    ...
    @classmethod
    def from_dict(cls, data: Dict) -> "CursorSession":
...
class SessionStorage:
    ...
    async def save_session(self, session: CursorSession) -> None:
        """Save session to storage."""
        raise NotImplementedError

    async def load_session(
        self, session_id: str, user_id: int
    ) -> Optional[CursorSession]:
    ...
    async def get_user_sessions(self, user_id: int) -> List[CursorSession]:
        """Get all sessions for a user."""
        raise NotImplementedError

    async def get_all_sessions(self) -> List[CursorSession]:
        """Get all sessions."""
        raise NotImplementedError


class SessionManager:
    """Manage Cursor Agent sessions."""
    ...
    async def get_or_create_session(
        ...
        # Create session with empty ID — Cursor will provide the real one
        new_session = CursorSession(
            session_id="",
            user_id=user_id,
        ...
        # Don't save to storage yet — deferred until after Cursor responds
        # with a real session_id (via update_session)

        logger.info(
            "Created new session (pending Cursor session ID)",
            user_id=user_id,
            project_path=str(project_path),
        )
    ...
    async def update_session(
        self, session: CursorSession, response: ClaudeResponse
    ) -> None:
        """Update session with response data and persist."""
        if session.is_new_session:
            # Assign the real session ID from Cursor
            if response.session_id:
                session.session_id = response.session_id
            else:
                logger.warning(
                    "Cursor returned no session_id for new session; "
            ...
            logger.info(
                "New session assigned Cursor session ID",
    ...
    async def _get_user_sessions(self, user_id: int) -> List[CursorSession]:
        """Get all sessions for a user."""
