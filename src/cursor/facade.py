"""High-level Cursor Agent integration facade.
...
class CursorIntegration:
    """Main integration point for Cursor Agent."""

    def __init__(
        self,
        config: Settings,
        cursor_manager: Optional[CursorAgentManager] = None,
        session_manager: Optional[SessionManager] = None,
    ):
        """Initialize Cursor integration facade."""
        self.config = config
        self.cursor_manager = cursor_manager or CursorAgentManager(config)
    ...
    async def run_command(
        ...
        on_stream: Optional[Callable[["ClaudeResponse"], None]] = None,
        force_new: bool = False,
        interrupt_event: Optional["asyncio.Event"] = None,
        images: Optional[List[Dict[str, str]]] = None,
    ) -> "ClaudeResponse":
        """Run Cursor Agent command with full integration."""
        logger.info(
        ...
        try:
            ...
            try:
                ...
                # If resume failed (e.g., session expired/missing on Cursor's side),
                ...
                if should_continue:
                    logger.warning(
                        ...
                        failed_session_id=cursor_session_id,
    ...
    async def _execute(
        ...
        stream_callback: Optional[Callable[["ClaudeResponse"], None]] = None,
        interrupt_event: Optional[asyncio.Event] = None,
        images: Optional[List[Dict[str, str]]] = None,
    ) -> "ClaudeResponse":
        """Execute command via Cursor CLI."""
        return await self.cursor_manager.execute_command(
            prompt=prompt,
            working_directory=working_directory,
            session_id=session_id,
    ...
    async def _find_resumable_session(
        ...
        user_id: int,
        working_directory: Path,
    ) -> Optional["CursorSession"]:  # noqa: F821
        ...
        (non-temporary) session ID from Cursor. Returns None otherwise.
        """
    ...
    async def continue_session(
        ...
        # Cursor CLI requires a prompt, so we use a placeholder
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("Shutting down Cursor integration")

        await self.cleanup_expired_sessions()

        logger.info("Cursor integration shutdown complete")
